...
以下のとおり、**Docker-Composeでフロントエンドとバックエンドを統一管理するモノレポ構成**で開発を進めました。 
...
### 構成の狙い
- 開発環境の統一化
Dockerを使用し、フロントエンド（React）とバックエンド（Rails）の開発環境を統一的に管理。
以下のように効率的なセットアップを実現しました。
 - **環境の一貫性**: **Dockerファイルで環境を管理し、ローカル環境と本番環境の設定差異によるエラーを防止。将来的なチーム開発における「自分の環境では動く」**といった問題も防止。
 - **環境構築の効率化**: Docker Composeによって一元的にアプリを起動・管理できる仕組みを整え、環境構築にかかる時間を短縮しすぐに開発作業に移れる。

- 明確な役割分担とコードベースの一元化
apiフォルダにバックエンドのコード、frontフォルダにフロントエンドのコードを格納。 
両者を1つのリポジトリに集約することで、以下のようにメンテナンス性を向上させました。
 - **変更の影響範囲の把握**: バックエンドAPIに変更が加わった際、フロントエンドの影響を即座に確認し修正可能なため開発効率が向上。  
 - **管理コストの削減**: リポジトリを1つに統一することで、複数のリポジトリを行き来する必要がなく、開発効率が向上。

### 使用技術
- **Docker** - コンテナ管理ツール
Dockerは、コンテナ環境を利用してアプリケーションを軽量かつ分離して実行可能にするツールです。
フロントエンドとバックエンドが異なる技術スタック（ReactとRails）で構築されている場合でも、同一の環境で簡単に実行可能です。
チームメンバー間で統一された開発環境を容易に共有できるため、実務での開発効率が大幅に向上することを意識して採用しました。

本プロジェクトは個人開発ですが、実務でのチーム開発を見据え、効率的な開発環境整備に向けた実践的なスキルの習得に挑戦しました。
