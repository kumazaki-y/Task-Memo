...
- **Emotion (11.13.3)** - スタイリングライブラリ
- **Axios (1.7.5)** - API通信用のHTTPクライアント
- **Jotai (2.9.2)** - 状態管理ライブラリ
- **React Hook Form (7.53.1)** - フォームのバリデーションと管理
- **Yup (1.4.0)** - スキーマベースのバリデーション
- **SWR (2.2.5)** - データフェッチングライブラリ
- **Vite (5.4.0)** - 開発ビルドツール
- **Vite TSConfig Paths (5.0.1)** - TypeScriptパス解決の補助プラグイン
- **Framer Motion (11.5.4)** - アニメーションライブラリ

### 開発支援ツール

- **ESLint (8.57.0)** - JavaScript/TypeScriptのコード品質管理
- **Prettier (3.3.3)** - コード整形ツール
- **Stylelint (16.8.1)** - CSSのコード品質管理
- **Jest (29.7.0)** - テストフレームワーク
- **Lint Staged (15.2.8)** - コードの自動整形（コミット前に適用）
- **Simple Git Hooks (2.11.1)** - Gitフックの簡略化
- **@typescript-eslint/eslint-plugin (6.4.0)** - TypeScript用のESLintプラグイン
- **ESLint Plugin React (7.35.0)** - React用のESLintプラグイン
- **ESLint Plugin JSX-A11y (6.9.0)** - アクセシビリティのためのESLintプラグイン
- **Stylelint Config Recess Order (5.0.1)** - CSSのプロパティ順序管理

## 使用技術（バックエンド）

- **Ruby (3.3.4)** - プログラミング言語
- **Rails (7.1.4)** - Webアプリケーションフレームワーク
- **MySQL (mysql2 ~> 0.5)** - データベース
- **Puma (6.4.2)** - アプリケーションサーバー
- **Devise (4.9.4)** - ユーザー認証機能
- **Devise Token Auth (1.2.3)** - トークンベースの認証
- **AWS Secrets Manager (1.108.0)** - 機密情報の管理
- **Dotenv (3.1.2)** - 環境変数の管理
- **Rack CORS (2.0.2)** - CORS設定（クロスオリジンリソース共有）
- **Rubocop (1.66.0)** - コード品質チェック
- **TZInfo-Data** - タイムゾーン情報の管理

### 説明
- **Ruby**と**Rails**は、アプリケーションの基盤となる開発フレームワークとプログラミング言語です。
- **MySQL**は、アプリケーションのデータを保存・管理するリレーショナルデータベースとして使用しています。
- **Puma**は、Railsアプリケーションのアプリケーションサーバーであり、リクエストの処理を効率化します。
- **Devise**と**Devise Token Auth**は、ユーザーの認証・トークン認証に用いており、APIベースのセキュアな認証機能を実現しています。
- **AWS Secrets Manager**は、APIキーや機密情報の安全な管理に使用されており、セキュリティの向上に寄与しています。
- **Dotenv**は、ローカル開発環境での環境変数管理に使用され、アプリケーションの設定情報を柔軟に管理できます。
- **Rack CORS**は、クライアント側からのリクエストが異なるオリジン（Vercelなど）から来た場合でも、APIが正常に機能するようCORS設定を行います。
- **Rubocop**は、コードのスタイルや品質をチェックし、メンテナンス性の高いコードの維持に役立てています。
- **TZInfo-Data**は、世界各地のタイムゾーンに対応するためのデータを提供します。
